    ...
    for(int i=0; i<rows.size(); i++) {
       if(rows.at(i)>maxGranica){
           if(i>ostatniDuzy+rowSzerokosc){
               atOCR_Wiersz wiersz=atOCR_Wiersz(ostatniDuzy, i, &imgGray,cnf);
...
void atOCR_Wiersz::analizaKolumn(){
    ...
    int colSzerokosc=cnf->getInt("colSzerokosc");
    double colPoziom=cnf->getFloat("colPoziom");
    ...
    for(int i=1; i<cols.size(); i++) {
       if(cols.at(i)>maxGranica){
           if(i>ostatniDuzy+colSzerokosc){
               ...
               atOCR_Pole pole=atOCR_Pole(rowMin,rowMax,ostatniDuzy, i, imgGray,cnf);
               pole.analiza();
               pola.push_back(pole);
           }
           ostatniDuzy=i;
...
void atOCR_Pole::analiza(){
    ...
    int paddingCol=cnf->getInt("paddingCol");
    int paddingRow=cnf->getInt("paddingRow");
